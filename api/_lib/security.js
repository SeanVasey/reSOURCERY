/**
 * Shared SSRF-hardening helpers for the api/ serverless endpoints.
 *
 * Vercel does not expose underscore-prefixed files under api/ as endpoints,
 * so this module is import-only. Both /api/fetch and /api/resolve rely on
 * the same validate → pin-IP → request contract to prevent DNS-rebinding
 * (TOCTOU between validation and connection).
 */

export const MAX_CONTENT_LENGTH = 2 * 1024 * 1024 * 1024; // 2 GB
export const MAX_REDIRECTS = 5;
export const CONNECT_TIMEOUT_MS = 30_000;     // 30s connection timeout
export const STREAM_IDLE_TIMEOUT_MS = 60_000; // 60s idle timeout during streaming
export const MAX_STREAM_TIME_MS = 5 * 60_000; // 5 min max total streaming time
export const DEFAULT_USER_AGENT = 'reSOURCERY/2.5 (+https://resourcery.app)';
 ...
 */
export function parseIP(hostname) {
 ...
 */
export async function validateTarget(target, res) {
 ...
 *
 * options.headers merge over the defaults (e.g. a custom User-Agent);
 * options.connectTimeoutMs overrides the connection timeout. Callers that
 * pass no options get the exact historical behavior.
 */
export async function pinnedRequest(urlString, pinnedIP, pinnedFamily, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    const req = mod.request({
      ...
      lookup: (_hostname, lookupOptions, callback) => {
        // Node ≥20 enables autoSelectFamily by default, which invokes
        // lookup with { all: true } and expects an address array; older
        // callers expect (err, address, family). Serve both shapes,
        // always answering only with the pre-validated pinned IP.
